...
template <class K>
void HashTable<K>::change_max_values() {
    max_values = {0, 0, 0};

    for (const K& key : keys) {
        compare_max_values(key, get_value(key));
    }
}
...
template <class K>
int HashTable<K>::get_value(const K& key) const {
    int hash_index = hash_f(key.name);
...
template <class K>
vector<K> HashTable<K>::get_keys() const {
    return keys;
};
